...
game_range = [2021020001, 2021020019]
...
if os.path.isfile(os.path.join(root,team_games_filename)):
    with open(os.path.join(root,teams_games_filename), 'r') as infile:
...
for game_id in range(game_range[0], game_range[1], 1):
...
path_teams_games = os.path.join(root, current_game)
with open(path_teams_games, 'w') as outfile:                
    json.dump(team_games_ids, outfile)